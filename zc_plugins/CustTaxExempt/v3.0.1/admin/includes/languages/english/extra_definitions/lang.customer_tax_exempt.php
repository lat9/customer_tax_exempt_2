<?php
//  Title: Customers Tax Exempt
//  Author: Jeff Lew
//  Contact: jlew@numinix.com
//  Last Update: 24/07/2007 by Jeff Lew
//  Organization: Numinix Technology <http://www.numinix.com>
//
// Part of the "Customer Tax-Exempt, v2" plugin by lat9
// Copyright (c) 2019-2026, Vinos de Frutas Tropicales
//
$define = [
    'ENTRY_TAX_EXEMPT' => 'Customer Tax Exempt',
    'NOTES_TAX_EXEMPT' => 'Either enter "ALL" (without quotes) if the customer is exempt from all taxes or copy/paste from the list of the store\'s current tax-descriptions (below), separated by commas.',
];
return $define;
